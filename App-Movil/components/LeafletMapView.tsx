...
interface LeafletMapProps {
  ...
  // Nuevas props para datos filtrados
  filteredParadas?: any[];
  filteredRutas?: any[];
  filteredBuses?: any[];
}
...
export const LeafletMapView: React.FC<LeafletMapProps> = ({
  ...
  const updateMapElements = () => {
    if (!webViewRef.current || !mapReady) {
      console.log('🗺️ updateMapElements: WebView no está listo', { webViewReady: !!webViewRef.current, mapReady });
      return;
    }
    ...
    console.log('🗺️ Actualizando elementos del mapa:', {
      showStops,
      showRoutes,
      showBuses,
    ...
    // Si es la primera carga (ningún filtro había cambiado antes), siempre actualizar
    const isInitialLoad = lastUpdate.showStops === undefined || lastUpdate.showRoutes === undefined || lastUpdate.showBuses === undefined;
    
    if (!isInitialLoad && !needsStopsUpdate && !needsRoutesUpdate && !needsBusesUpdate && !origin && !destination && !plannedRoute) {
      console.log('🗺️ No hay cambios que actualizar en el mapa');
      return;
    }
    ...
    let javascript = `
      ...
      window.stopsData = ${JSON.stringify(dataToUse.paradas)};
    `;

    // Siempre limpiar marcadores existentes para evitar duplicados
    javascript += `
      // Limpiar todos los marcadores y rutas existentes
    ...
    if (showStops) {
      javascript += `
        console.log('=== AGREGANDO PARADAS AL MAPA ===');
        console.log('Datos de paradas:', window.stopsData ? window.stopsData.length : 0, 'elementos');
        if (window.stopsData && window.stopsData.length > 0) {
          console.log('Primeras 3 paradas para verificar estructura:', window.stopsData.slice(0, 3));
          let paradasAgregadas = 0;
          window.stopsData.forEach((stop, index) => {
            try {
              if (window.addStop) {
                window.addStop(stop);
                paradasAgregadas++;
              ...
              console.error('Error agregando parada', index, ':', error, 'Datos:', stop);
            }
          });
          console.log('Total paradas agregadas al mapa:', paradasAgregadas);
        } else {
          console.warn('No hay datos de paradas disponibles para el mapa');
        }
      `;
    }
    ...
    if (showRoutes) {
      javascript += `
        const routesData = ${JSON.stringify(dataToUse.rutas)};
        console.log('=== AGREGANDO RUTAS AL MAPA ===');
        console.log('Datos de rutas:', routesData.length, 'elementos');
        if (routesData && routesData.length > 0) {
          console.log('Primeras 3 rutas para verificar estructura:', routesData.slice(0, 3));
          window.routesData = routesData;
          let rutasAgregadas = 0;
          routesData.forEach((route, index) => {
            try {
              if (window.addRoute) {
                window.addRoute(route, index);
                rutasAgregadas++;
              }
            } catch (error) {
              console.error('Error agregando ruta', index, ':', error, 'Datos:', route);
            }
          });
          console.log('Total rutas agregadas al mapa:', rutasAgregadas);
        } else {
          console.warn('No hay datos de rutas disponibles para el mapa');
        }
      `;
    }
    ...
    if (showBuses) {
      javascript += `
        const busesData = ${JSON.stringify(dataToUse.buses)};
        console.log('=== AGREGANDO BUSES AL MAPA ===');
        console.log('Datos de buses:', busesData.length, 'elementos');
        if (busesData && busesData.length > 0) {
          console.log('Primeros 3 buses para verificar estructura:', busesData.slice(0, 3));
          let busesAgregados = 0;
          busesData.forEach((bus, index) => {
            try {
              ...
              console.error('Error agregando bus', index, ':', error, 'Datos:', bus);
            }
          });
          console.log('Total buses agregados al mapa:', busesAgregados);
        } else {
          console.warn('No hay datos de buses disponibles para el mapa');
        }
      `;
    }
  ...
  // Actualizar datos en el mapa cuando cambian los filtros o los datos filtrados
  useEffect(() => {
    if (!mapReady) return;
    updateMapElements();
  }, [
  ...
  // Actualizar cuando cambien los datos filtrados (usar JSON.stringify para comparación profunda)
  useEffect(() => {
    if (!mapReady) return;
    const dataParadas = filteredParadas || paradas;
    const dataRutas = filteredRutas || rutas;
    const dataBuses = filteredBuses || buses;
    
    if (dataParadas.length > 0 || dataRutas.length > 0 || dataBuses.length > 0) {
      console.log('🚀 Carga inicial del mapa con datos:', {
        paradas: dataParadas.length,
        rutas: dataRutas.length,
        buses: dataBuses.length,
        filtered: !!(filteredParadas || filteredRutas || filteredBuses)
      });
      updateMapElements();
    }
  }, [
    mapReady, 
    JSON.stringify(filteredParadas?.map(p => p.id) || paradas.map(p => p.id)),
    JSON.stringify(filteredRutas?.map(r => r.id) || rutas.map(r => r.id)),
    JSON.stringify(filteredBuses?.map(b => b.id) || buses.map(b => b.id))
  ]);
