...
export const FavoritesManager: React.FC = () => {
  ...
  const { favorites, loading: favoritesLoading, addToFavorites, removeFromFavorites, isFavorite, getFavoritesByType } = useFavorites();
  ...
  // Obtener favoritos del contexto actualizado
  const favoriteStopItems = getFavoritesByType('parada');
  const favoriteRouteItems = getFavoritesByType('ruta');
  
  // Encontrar los objetos completos de paradas y rutas
  const favoriteStops = paradas.filter(parada => 
    favoriteStopItems.some(fav => fav.itemId === parada.id)
  );

  const favoriteRoutes = rutas.filter(ruta => 
    favoriteRouteItems.some(fav => fav.itemId === ruta.id)
  );
  ...
  const handleToggleStopFavorite = async (stopId: string) => {
    ...
    try {
      const parada = paradas.find(p => p.id === stopId);
      if (!parada) {
        throw new Error('Parada no encontrada');
      }

      if (isFavorite(stopId, 'parada')) {
        await removeFromFavorites(stopId, 'parada');
      } else {
        await addToFavorites(parada, 'parada');
      }
    } catch (error) {
      Alert.alert('Error', 'No se pudo actualizar los favoritos');
      console.error('Error updating stop favorite:', error);
    } finally {
  ...
  const handleToggleRouteFavorite = async (routeId: string) => {
    ...
    try {
      ...
      if (isFavorite(routeId, 'ruta')) {
        await removeFromFavorites(routeId, 'ruta');
      } else {
        await addToFavorites(ruta, 'ruta');
      }
    } catch (error) {
      Alert.alert('Error', 'No se pudo actualizar los favoritos');
    ...
    <ScrollView showsVerticalScrollIndicator={false}>
      ...
      <Card margin="md" variant="outlined">
        <ThemedView style={styles.infoContainer}>
          <Icon name="information-circle-outline" library="ionicons" size="md" color="primary" />
