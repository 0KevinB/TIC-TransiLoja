...
import { SimpleOfflineMapView } from './SimpleOfflineMapView';
import { shouldUseGoogleMaps } from '../lib/platform';
...
export const MapComponent: React.FC<MapComponentProps> = (props) => {
  ...
  // Si está offline, usar vista simplificada
  if (isOffline) {
    return (
      <View style={{ flex: 1 }}>
        <SimpleOfflineMapView
  ...
  // Si está online, determinar qué componente de mapa usar basado en el entorno
  const MapViewComponent = shouldUseGoogleMaps() ? GoogleMapsView : LeafletMapView;
